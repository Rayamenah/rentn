...
export function userSchemaValidation(data: any) {
  const userSignUpValidation = zod.object({
    email: zod.string().nonempty('email must not be left empty').email().trim(),
    name: zod
      .string()
...
export function apartmentSchemaValidation(data: {}) {
  const apartmentValidation = zod.object({
    description: zod
      ...
      .min(
        10,
        'please provide more description about the apartment you want to rent'
      )
    ...
    address: zod
      ...
      .min(
        10,
        'please provide more description to the address you want to rent'
      )
    ...
    community: zod
      ...
      .min(
        5,
        'please provide clarity about the community where the apartment is located'
      )
    ...
    apartmentType: zod
      ...
      .trim(),
    images: zod.string().optional().array(),
    price: zod.number(),
...
export function priceSchemaValidation(data: {}) {
  const priceSchema = zod.object({
    price: zod.number().nonnegative('please add a valid number'),
    tenure: zod.number().min(1, 'not less than 1').nonnegative(),
  })
  try {
